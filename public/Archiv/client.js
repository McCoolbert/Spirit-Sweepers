...
window.onload = function () {
  ...
  // Deine Zuweisungen HIER:
  players[0].name = document.getElementById('player1-name').value || "Spieler 1";
  players.color = document.getElementById('player1-color').value || "#d13b77";
  players.name = document.getElementById('player2-name').value || "Spieler 2";
  players.color = document.getElementById('player2-color').value || "#39ff14";
...
function initializeGame() {
  ...
  player.x = 2;
  player.y = 2;
  player.dir = 'right'; // oder Standardrichtung, wie du möchtest 
  player.score = 0;
  player.lives = 5;     // Startwert!
  player.cooldown = 0;
  player.invuln = 0;
  player.trap = null;
  player.speedTimer = 0;
  ...
  player.color = players[currentPlayerIndex].color;
...
for (let y = 0; y < HEIGHT; y++) {
  ...
  ghosts = [
    { ...randomEmptyPosition(), dx: 1, dy: 0, cooldown: 0, moveCooldown: 0 },
    { ...randomEmptyPosition(), dx: 0, dy: -1, cooldown: 0, moveCooldown: 0 }
  ];
  powerups = [];
  floatingTexts = [];
  catchCombo = 0;
...
function endGame(timerEnd) {
  ...
  // Score beim aktuellen Spieler speichern:
  players[currentPlayerIndex].score = [player.score];

  if (currentPlayerIndex === 0) {
  setTimeout(() => {
    alert(players.name + " ist fertig. Jetzt ist " + players.name + " dran!");
    ...
    // Nach Runde 2: Highscore-Overlay wie gewohnt anzeigen
    setTimeout(() => {
      ...
      // Zusatzanzeige für beide Spieler einbauen (siehe nächster Schritt)
    }, 200);
  }
}
...
function showHighscoreOverlay(playerName, playerScore, gameOverReason) {
  // Spieler-Vergleich einblenden, wenn beide Runden gespielt wurden
  let playerScoreInfo = "";
if (
  typeof players !== 'undefined' &&
  players.length === 2 &&
  typeof players[0].score === 'number' &&
  typeof [players.score] === 'number'
) {
  playerScoreInfo =
    `<div style="color:#ffeafd; margin-bottom:7px; font-size:1em;">
      ${[players.name]}: <b>${[players.score]}</b>
      &nbsp;—&nbsp;
      ${[players.name]}: <b>${[players.score]}</b>
    </div>
    <div style="color:#ffd3df;font-size:1em;margin-bottom:10px;">
      <b>${
        [players.score] > [players.score]
          ? `${[players.name]} gewinnt!`
          : [players.score] > [players.score]
          ? `${[players.name]} gewinnt!`
          : "Unentschieden!"
      }</b>
  ...
  // Schreibe das ins Overlay unter deinen Highscore-Block
  document.getElementById('player-score-row').innerHTML = playerScoreInfo;
  ...
  if (!playerInTop && !highscores.find(h => h.name === playerName)) {
    // Hier NICHT nochmal player-score-row überschreiben,
    // sondern optional dem User eine Zusatzinfo geben!
    // Der folgende Block kann gern entfernt werden:
  ...
  // UND: der andere else-Zweig auch raus, sonst verschwindet die Übersicht
  // else {
  //   document.getElementById('player-score-row').innerHTML = '';
  // }
...
document.getElementById('btn-retry').onclick = function() {
  ...
  // Canvas entfernen (falls vorhanden)
  if (canvas && canvas.parentNode) {
    canvas.parentNode.removeChild(canvas);
  }

  // 2-Spieler-Daten zurücksetzen & Spiel restart
